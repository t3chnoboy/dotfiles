...
var hoverZoom = {
    ...
    loadHoverZoom:function () {
        ...
        function hideHoverZoomImg(now) {
            ...
            //chrome.extension.sendMessage({action: 'viewWindow', visible: false});
        }
        ...
        function displayFullSizeImage() {
            ...
            if (options.addToHistory && !chrome.extension.inIncognitoContext) {
                ...
                chrome.extension.sendMessage({action:'addUrlToHistory', url:url});
            }
            chrome.extension.sendMessage({action:'trackEvent', event:{category:'Actions', action:'ImageDisplayedOnSite', label:document.location.host}});
            chrome.extension.sendMessage({action:'trackEvent', event:{category:'Actions', action:'ImageDisplayedFromSite', label:imgDetails.host}});
        }
        ...
        function imgFullSizeOnError() {
            if (imgDetails.url == $(this).attr('src')) {
                ...
                if (hz.currentLink && hoverZoomSrcIndex < hz.currentLink.data().hoverZoomSrc.length - 1) {
                    ...
                    chrome.extension.sendMessage({action:'trackEvent', event:{category:'Errors', action:'LoadingErrorFromSite', label:imgDetails.host}});
                }
            }
        }
        ...
        function prepareImgLinks() {
            ...
            if (options.alwaysPreload) {
                ...
                chrome.extension.sendMessage({action:'preloadAvailable'});
            }
        ...
        function loadOptions() {
            chrome.extension.sendMessage({action:'getOptions'}, function (result) {
                options = result;
                if (options) {
                applyOptions();
                }
        ...
        function openImageInWindow() {
            chrome.extension.sendMessage({action:'getItem', id:'popupBorder'}, function (data) {
                var createData,
    ...
    preloadImages:function () {
        ...
        function preloadNextImage() {
            ...
            if (link.data().hoverZoomPreloaded) {
                preloadNextImage();
                chrome.extension.sendMessage({action:'preloadProgress', value:preloadIndex, max:links.length});
            } else {
