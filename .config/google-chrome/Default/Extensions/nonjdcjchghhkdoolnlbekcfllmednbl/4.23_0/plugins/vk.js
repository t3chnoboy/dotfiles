...
hoverZoomPlugins.push({
    ...
    prepareImgLinks:function (callback) {
        ...
        function prepareFromPhotoId(link, photoId, listId) {
            ...
            chrome.extension.sendMessage({action:'ajaxRequest',
        ...
        $('img[src*="/u"]').filter(function () {
            return this.src.match(/\/u\d+\/[ed]_/);
                ...
                chrome.extension.sendMessage({action:'ajaxGet', url:'http://vk.com/al_profile.php?al=1&act=get_profile_photos&offset=0&skip_one=0&id=' + userId}, function (response) {
                    var photos;
                    try {
                        photos = JSON.parse(response.match(/<!json>(.*)$/)[1]);
                    } catch (e) {
