...
function init(optionsSynced) {
    ...
    options = optionsSynced || defaultOptions;
    ...
    for (var key in options) {
      ...
      } else if (options[key] && (el = byId(key))) {
...
function save() {
    ...
    AU.support.toggle(byId('adSupport').checked)
    AU.support.toggleFish(byId('productSearchSupport').checked)
}
...
AU.init({background: false}, function() {
  byId('adSupport').checked = AU.support.enabled();
  byId('productSearchSupport').checked = AU.support.fishEnabled();
})
